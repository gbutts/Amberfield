 ...
 */
function check_login($dbc, $uname = '', $pass = '') {
 global $dbc, $r;
	...
	if (empty($uname)) {
		...
		$u = mysqli_real_escape_string($dbc, trim($uname));
	}
	...
	if (empty($errors)) { // If everything's OK.
		...
		$q = "SELECT user_id, fname FROM users WHERE uname='$u' AND pass=SHA1('$p')";
