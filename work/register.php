<?php # Script 8.5 - register5.php
    session_start(); // Start the session.
    $pname = 'Register an Animal on the Database';
    $version  = 'v4.1.0'; //Adds parents
...
if (isset($_POST['submitted'])) {
	...
	// Check for a sire.
	if ($_POST['sire_id'] == 'Select...' ) {
		$errors[] = 'You forgot to enter a Sire.';
	} else {
		$sr = mysqli_real_escape_string($dbc, trim($_POST['sire_id']));
	}

	// Check for a dam.
	if ($_POST['dam_id'] == 'Select...' ) {
		$errors[] = 'You forgot to enter a Dam.';
	} else {
		$dm = mysqli_real_escape_string($dbc, trim($_POST['dam_id']));
	}
	...
	if (empty($errors)) { // If everything's OK.
        ...
        // Make the query:
    		$q1 = "INSERT INTO animal (aname, stud_id, tattoo, dob, sex, sireo, damo, role_id) 
    			VALUES ('$an', '$si', '$t', '$dob', '$s', '$sr', '$dm', '$ro')";
...
<h1>Register a Stud Animal</h1>
  ...
  <fieldset>
   <legend>Fill in the animal's details</legend><br />
    <table summary="form" align="center" border="0" width ="400px">
      ...
      <tr>
        ...
        <td  valign= "middle"><input type="date" name="dob" size="15" maxlength="10" value="<?php if (isset($_POST['dob'])) echo $_POST['dob']; ?>" />&nbsp;&nbsp;<a href="javascript:showCal('Calendar1')" style="text-decoration:none"><img src="images/calendar.jpg" width="16" height="15" border="0"  /></a>
		</td>
      </tr>
      <tr>
      ...
      <tr>
  	    <td align="right" >Sire:</td>
	    <td valign="middle"><select name="sire_id" STYLE="width: 120px"><option selected>Select...</option>
	    <?php dropdown6('animal_id', 'aname', 'animal', 'M'); ?>
	    </select></td>
      </tr>
      <tr>
  	    <td align="right" >Dam:</td>
	    <td valign="middle"><select name="dam_id" STYLE="width: 120px"><option selected>Select...</option>
	    <?php dropdown6('animal_id', 'aname', 'animal', 'F'); ?>
	    </select></td>
      </tr>
      <tr>
        <td><br /><br /><br /></td>
